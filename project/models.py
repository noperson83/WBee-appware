...
class ProjectTemplate(TimeStampedModel):
    ...
    business_category = models.ForeignKey(
        BusinessCategory, on_delete=models.CASCADE, related_name="project_templates"
    )
    ...
    template_tasks = models.JSONField(
        default=list, blank=True, help_text="Default tasks for this project type"
    )
    required_materials = models.JSONField(
        default=list, blank=True, help_text="Common materials needed"
    )
...
class Project(UUIDModel, TimeStampedModel):
    ...
    revision = models.CharField(
        max_length=50, blank=True, help_text="Project revision/version"
    )
    ...
    description = models.TextField(
        max_length=4000, blank=True, help_text="Detailed project description"
    )
    scope_overview = models.TextField(
        max_length=2000, blank=True, help_text="High-level scope summary"
    )
    ...
    site_contact = models.TextField(
        max_length=500, blank=True, help_text="On-site contact information"
    )
    ...
    date_requested = models.DateField(
        null=True, blank=True, help_text="Date project was requested"
    )
    ...
    invoiced_amount = models.DecimalField(
        max_digits=20, decimal_places=2, default=0, help_text="Amount invoiced to date"
    )
    paid_amount = models.DecimalField(
        max_digits=20, decimal_places=2, default=0, help_text="Amount paid to date"
    )
    paid_date = models.DateField(
        null=True, blank=True, help_text="Date final payment received"
    )
    ...
    tax_status = models.CharField(
        max_length=50, default="taxable", help_text="Tax classification (configurable)"
    )
    ...
    custom_fields = models.JSONField(
        default=dict, blank=True, help_text="Custom project data"
    )
    ...
    pricing_disclaimer = models.TextField(
        max_length=2000, blank=True, help_text="Pricing terms and disclaimers"
    )
    ...
    def get_absolute_url(self):
        ...
        return reverse(
            "project:project-detail", kwargs={"job_number": str(self.job_number)}
        )
    ...
    def profit_margin(self):
        ...
        if self.contract_value and self.estimated_cost and self.estimated_cost > 0:
            return (
                (self.contract_value - self.estimated_cost) / self.contract_value
            ) * 100
        return 0
...
class ScopeOfWork(TimeStampedModel):
    ...
    area = models.CharField(
        max_length=200, help_text="Area or location within the project"
    )
    ...
    phase = models.CharField(
        max_length=100, blank=True, help_text="Project phase (Phase 1, Phase 2, etc.)"
    )
...
class ProjectMaterial(TimeStampedModel):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="material_items"
    )
    ...
    scope_item = models.ForeignKey(
        ScopeOfWork, on_delete=models.SET_NULL, null=True, blank=True
    )
    ...
    material_type = models.CharField(
        max_length=20, choices=MATERIAL_TYPES, default="device"
    )
...
class ProjectChange(TimeStampedModel):
    ...
    project = models.ForeignKey(
        Project, on_delete=models.CASCADE, related_name="changes"
    )
    change_type = models.CharField(
        max_length=50
    )  # scope_change, budget_change, schedule_change
    description = models.TextField()
