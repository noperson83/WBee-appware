...
class ProjectDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def _build_dashboard_data(self, user):
        ...
        # Recent projects
        recent_projects = (
            base_queryset.select_related("primary_location", "project_manager")
    ...
    def _get_user_projects(self, user):
        ...
        if user.role == "admin":
            ...
            return Project.objects.filter(
                Q(project_manager=user) | Q(estimator=user) | Q(team_leads=user)
            ).distinct()
        elif user.role in ["supervisor", "worker"]:
            return Project.objects.filter(
                Q(supervisor=user) | Q(team_leads=user) | Q(team_members=user)
            ).distinct()
        elif user.role == "client":
            if hasattr(user, "client"):
                return Project.objects.filter(primary_location__client=user.client)
    ...
    def _get_user_notifications(self, user):
        ...
        # Overdue projects
        overdue_count = (
            self._get_user_projects(user)
            .filter(due_date__lt=date.today(), status__in=["active", "installing"])
            .count()
        )
        ...
        if user.role in ["admin", "project_manager"]:
            pending_changes = ProjectChange.objects.filter(
                project__in=self._get_user_projects(user), is_approved=False
            ).count()
    ...
    def _calculate_team_performance(self, projects):
        ...
        return {
            "total_team_members": User.objects.filter(assigned_projects__in=projects)
            .distinct()
            .count(),
            "active_team_leads": projects.values("team_leads").distinct().count(),
            "avg_project_duration": projects.filter(completed_date__isnull=False)
            .annotate(duration=F("completed_date") - F("start_date"))
            ...
            avg_profit_margin=Avg(
                Case(
                    When(
                        ...
                        then=(F("contract_value") - F("estimated_cost"))
                        * 100
                        / F("contract_value"),
                    ),
                    default=Value(0),
...
class ProjectListView(LoginRequiredMixin, OptimizedQuerysetMixin, ListView):
    ...
    select_related_fields = [
        "primary_location",
        "project_manager",
        # Follow the relation through ``primary_location`` to load the project's
        # business category in a single query.
        "primary_location__business_category",
    ]
    ...
    def get_queryset(self):
        ...
        if user.role == "admin":
            ...
            if hasattr(user, "client"):
                queryset = queryset.filter(primary_location__client=user.client)
            else:
        ...
        if search_query:
            queryset = queryset.filter(
                ...
                | Q(primary_location__client__company_name__icontains=search_query)
            ).distinct()
        ...
        context.update(
            {
                "status_choices": (
                    ...
                    if hasattr(Project._meta.get_field("status"), "choices")
                    else []
                ),
...
class ProjectDetailView(ProjectAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["task_list"] = TaskList.objects.filter(project=project).select_related(
            "scope"
        )
        ...
        context["upcoming_milestones"] = project.milestones.filter(
            target_date__gte=date.today(), is_complete=False
    ...
    def _get_user_permissions(self, project, user):
        ...
        if user.role == "project_manager":
            permissions.update(
                {
                    "can_edit": project.project_manager == user
            ...
            permissions.update(
                {
                    "can_edit": project.supervisor == user
    ...
    def _get_recent_activity(self, project):
        ...
        recent_milestones = project.milestones.filter(
            is_complete=True, actual_date__isnull=False
...
class ProjectCreateView(LoginRequiredMixin, AjaxResponseMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if not form.instance.job_number:
            form.instance.job_number = generate_job_number(
                form.instance.primary_location.business_category
                if form.instance.primary_location
                else None
            )
        ...
        if self.request.user.role == "project_manager":
            ...
            if active_projects >= max_projects:
                form.add_error(
                    None,
                    f"You have reached your maximum concurrent project limit ({max_projects})",
                )
            ...
            for task_data in project.template.template_tasks:
                if task_data.get("is_milestone"):
                    ProjectMilestone.objects.create(
                        ...
                        target_date=(
                            project.start_date
                            + timedelta(days=task_data.get("days_offset", 0))
    ...
    def get_success_url(self):
        return reverse(
            "project:project-detail", kwargs={"job_number": self.object.job_number}
        )


class ProjectUpdateView(
    ProjectAccessMixin, ProjectPermissionMixin, AjaxResponseMixin, UpdateView
):
    ...
    def _handle_status_change(self, old_instance, new_instance):
        ...
        if old_instance.status != new_instance.status:
            if new_instance.status == "complete" and not new_instance.completed_date:
                ...
                new_instance.status != "complete" and old_instance.status == "complete"
            ):
...
class ProjectDuplicateView(ProjectAccessMixin, ProjectPermissionMixin, CreateView):
    ...
    def get_initial(self):
        ...
        initial = model_to_dict(
            original,
            exclude=["id", "pk", "job_number", "revision", "created_at", "updated_at"],
        )
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'Project "{self.object.name}" duplicated successfully.'
        )
...
class ProjectStatusUpdateView(ProjectAccessMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(
            request,
            "project/project_status_form.html",
            {"form": form, "project": self.project},
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return redirect(
                "project:project-detail", job_number=self.project.job_number
            )
        return render(
            request,
            "project/project_status_form.html",
...
class OverdueProjectListView(ProjectListView):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(
            due_date__lt=date.today(), status__in=["active", "installing", "quoted"]
        )
...
class ProjectProgressView(ProjectAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        scope_progress = project.scope_items.annotate(
            ...
            "area", "system_type", "percent_complete", "task_count", "completed_tasks"
        )
            ...
            milestone_data.append(
                {
                    ...
                    "days_variance": (
                        (milestone.actual_date - milestone.target_date).days
                        if milestone.actual_date and milestone.target_date
                        else None
                    ),
                }
            )
    ...
    def _calculate_time_tracking(self, project):
        ...
        return {
            ...
            "time_progress_percentage": (
                (elapsed_days / total_days * 100) if total_days else 0
            ),
            "is_on_schedule": (
                project.percent_complete >= (elapsed_days / total_days * 100)
                if total_days
                else True
            ),
        }
...
class ProjectFinancialView(ProjectAccessMixin, DetailView):
    ...
    def _get_profit_analysis(self, project):
        ...
        if project.contract_value and project.estimated_cost:
            ...
            analysis["net_profit"] = (
                analysis["gross_profit"] * 0.85
            )  # Approximate after overhead
            analysis["roi_percentage"] = (
                analysis["gross_profit"] / project.estimated_cost
            ) * 100
            analysis["break_even_point"] = (
                project.estimated_cost / project.contract_value * 100
            )
...
class ScopeOfWorkCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:scope-list", kwargs={"job_number": self.project.job_number}
        )
...
class ScopeOfWorkUpdateView(ProjectAccessMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:scope-list", kwargs={"job_number": self.object.project.job_number}
        )
...
class ProjectDeviceCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:device-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectDeviceUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:device-list", kwargs={"job_number": self.object.project.job_number}
        )
...
class ProjectHardwareListView(ProjectAccessMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["create_url"] = reverse(
            "project:hardware-create", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectHardwareCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:hardware-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectHardwareUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            ...
            kwargs={"job_number": self.object.project.job_number},
        )
...
class ProjectSoftwareListView(ProjectAccessMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["material_type"] = "Software"
        context["create_url"] = reverse(
            "project:software-create", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectSoftwareCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:software-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectLicenseListView(ProjectAccessMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["material_type"] = "License"
        context["create_url"] = reverse(
            "project:license-create", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectLicenseCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:license-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectTravelListView(ProjectAccessMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["material_type"] = "Travel"
        context["create_url"] = reverse(
            "project:travel-create", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectTravelCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:travel-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectTravelUpdateView(UpdateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:travel-list", kwargs={"job_number": self.object.project.job_number}
        )
...
class ProjectChangeCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:change-list", kwargs={"job_number": self.project.job_number}
        )
    ...
    def post(self, request, pk):
        ...
        if action == "approve":
            ...
            if change.cost_impact:
                change.project.estimated_cost = (
                    change.project.estimated_cost or 0
                ) + change.cost_impact
                change.project.save()
...
class ProjectMilestoneCreateView(ProjectAccessMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse(
            "project:milestone-list", kwargs={"job_number": self.project.job_number}
        )
...
class ProjectMilestoneCompleteView(ProjectAccessMixin, View):
    ...
    def post(self, request, pk):
        ...
        return redirect(
            "project:milestone-list", job_number=milestone.project.job_number
        )
...
class ProjectTeamView(ProjectAccessMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        available_members = User.objects.filter(
            Q(roles__contains=["worker"]) | Q(roles__contains=["supervisor"])
        ).exclude(id__in=project.team_members.values_list("id", flat=True))

        # Available team leads
        available_leads = User.objects.filter(
            Q(roles__contains=["supervisor"]) | Q(roles__contains=["project_manager"])
        ).exclude(id__in=project.team_leads.values_list("id", flat=True))
...
class MyProjectAssignmentsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .distinct()
            .select_related("primary_location", "project_manager")
            .order_by("-updated_at")
        )
...
def project_search_api(request):
    ...
    projects = Project.objects.filter(
        ...
        | Q(primary_location__name__icontains=query)
    )[:limit]
        ...
        results.append(
            {
                ...
                "name": project.name,
                "primary_location": (
                    project.primary_location.name if project.primary_location else ""
                ),
                "status": project.status,
                "url": reverse(
                    "project:project-detail", kwargs={"job_number": project.job_number}
                ),
            }
        )
    ...
    if not stats:
        ...
        stats = {
            ...
            "overdue_projects": projects.filter(
                due_date__lt=date.today(), status__in=["active", "installing"]
            ).count(),
        }
...
def project_quick_update_api(request, job_number):
    ...
    # Check permissions
    if (
        not request.user.role in ["admin", "project_manager"]
        and project.project_manager != request.user
    ):
...
class ProjectDetailAPIView(
    ProjectAccessMixin, ProjectPermissionMixin, generics.RetrieveAPIView
):
...
class FinancialReportView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        top_projects = (
            queryset.filter(contract_value__isnull=False, estimated_cost__isnull=False)
            .annotate(
    ...
    def get_context_data(self, **kwargs):
        ...
        projects = Project.objects.select_related("project_manager").annotate(
            ...
            completed_milestones=Count(
                "milestones", filter=Q(milestones__is_complete=True)
            ),
        )
        ...
        progress_stats = {
            ...
            "behind_schedule": projects.filter(
                due_date__lt=date.today() + timedelta(days=30), percent_complete__lt=75
            ).count(),
            "avg_completion": projects.aggregate(avg=Avg("percent_complete"))["avg"]
            or 0,
        }
        ...
        completion_ranges = [
            ...
            (
                "25-50%",
                projects.filter(
                    percent_complete__gte=25, percent_complete__lt=50
                ).count(),
            ),
            (
                "50-75%",
                projects.filter(
                    percent_complete__gte=50, percent_complete__lt=75
                ).count(),
            ),
        ...
        manager_performance = (
            ...
            .annotate(
                ...
                on_time_projects=Count(
                    "id", filter=Q(completed_date__lte=F("due_date"))
                ),
            )
        ...
        team_stats = (
            ...
            .annotate(
                active_projects=Count(
                    "assigned_projects",
                    filter=Q(assigned_projects__status__in=["active", "installing"]),
                ),
...
class ProjectPDFExportView(ProjectAccessMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        params = {"project": project, "today": date.today(), "user": request.user}
        ...
        return JsonResponse(
            {
                "valid": not exists,
                "message": (
                    "Job number already exists" if exists else "Job number is available"
                ),
            }
        )
            ...
            [
                {
                    ...
                    "url": reverse("project:my-assignments"),
                    "icon": "fa-tasks",
                    "color": "info",
                },
                {
...
class RecentlyViewedProjectsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        return (
            Project.objects.filter(
                Q(project_manager=self.request.user) | Q(team_members=self.request.user)
            )
            .distinct()
            .order_by("-updated_at")[:10]
        )
...
def project_calendar_events_api(request):
            ...
            events.append(
                {
                    ...
                    "url": reverse(
                        ...
                        kwargs={"job_number": project.job_number},
                    ),
                    "color": "#dc3545",
                    "textColor": "white",
                }
    ...
    if event_type in ["milestones", "all"]:
        ...
        milestones = ProjectMilestone.objects.filter(
            project__in=user_projects, target_date__isnull=False
        )
            ...
            events.append(
                {
                    ...
                    "url": reverse(
                        "project:project-detail",
                        kwargs={"job_number": milestone.project.job_number},
                    ),
...
class ProjectHealthCheckView(View):
    ...
    def _check_cache(self):
        ...
        try:
            ...
            return {
                "status": value == "ok",
                "message": "Cache functional" if value == "ok" else "Cache not working",
            }
        except Exception as e:
