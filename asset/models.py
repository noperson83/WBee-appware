...
class AssetCategory(TimeStampedModel):
    ...
    name = models.CharField(max_length=100, help_text='Vehicle, Tool, Equipment, Furniture, etc.')
    ...
    default_depreciation_years = models.PositiveIntegerField(
        default=5,
        help_text='Default depreciation period in years'
    )
...
class Asset(UUIDModel, TimeStampedModel):
    ...
    year = models.PositiveIntegerField(null=True, blank=True, help_text='Year manufactured')
    ...
    is_billable = models.BooleanField(
        default=True,
        help_text='Can this asset be billed to clients?'
    )
    ...
    license_plate = models.CharField(max_length=20, blank=True, help_text='License plate (vehicles)')
    registration_expiration = models.DateField(null=True, blank=True)
    
    # Custom fields for business-specific data
    custom_fields = models.JSONField(default=dict, blank=True, help_text='Custom asset data')
    ...
    def is_available(self):
        ...
        return (self.status == 'available' and 
                self.condition not in ['needs_repair', 'out_of_service'] and
                self.is_active and not self.is_retired)
...
def create_default_asset_categories():
    ...
    try:
        ...
        for category, assets in [
            ...
            for name, icon, color, depreciation_years, requires_maintenance, maintenance_days in assets:
