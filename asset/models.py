...
class Asset(UUIDModel, TimeStampedModel):
    ...
    model = models.CharField(
        max_length=100, blank=True, help_text="Model number/name"
    )
    ...
    description = models.TextField(
        blank=True, help_text="Detailed description"
    )
    ...
    purchase_date = models.DateField(
        null=True, blank=True, help_text="Date purchased"
    )
    ...
    def depreciated_value(self):
        ...
        if self.purchase_price and self.purchase_date:
            ...
            total_depreciation = annual_depreciation * Decimal(
                str(years_owned)
            )

            return max(
                self.purchase_price - total_depreciation, Decimal("0.00")
            )
    ...
    def depreciation_rate(self):
        ...
        if self.purchase_price and self.purchase_price > 0:
            return (
                self.purchase_price / self.category.default_depreciation_years
            )
        return Decimal("0.00")
...
class AssetMaintenanceRecord(UUIDModel, TimeStampedModel):
    ...
    def __str__(self):
        return (
            ...
            f"{self.maintenance_type} on {self.performed_date}"
        )
...
def create_default_asset_categories():
    ...
    try:
        ...
        distribution = BusinessCategory.objects.get(
        ...
        print(
            "Business categories not found. "
            "Please run location data creation first."
        )
